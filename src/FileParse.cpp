    ...
    while( !bEndMonster && fgets( szRaw, 1024, m_fp ) != NULL )
    {
        ...
        if( !bEndMonster )
        {
            ...
            else if( strncasecmp( szLine, "attack", 6 ) == 0 )
            {
                ...
                {
                    JLog( LOG_LEVEL_INFO, "error parsing attack: effect type not found %s\n", cur );
                    continue;
                }
                *end++ = NULL;
                begin++;
                ...
                if( begin == NULL || end == NULL )
                {
                    JLog( LOG_LEVEL_INFO, "error parsing attack: attack type not found %s\n", cur );
                    continue;
                }
                *end++ = NULL;
                begin++;
                ...
                if( begin == NULL )
                {
                    JLog( LOG_LEVEL_INFO, "error parsing attack: damage not found %s\n", cur );
                    continue;
                }
                begin++;
            ...
            else if( strncasecmp( szLine, "color", 5 ) == 0 )
            {
                ...
                if( strchr( color, '<' ) != NULL )
                {
                    // multi-hued
                    // <<rgb1>,<rgb2>,...,<rgbn>>
                    JLog( LOG_LEVEL_INFO, "Found multi-hued monster: %s\n", color );
            ...
            {
                JLog( LOG_LEVEL_INFO, "Unparseable line:%s\n", szLine );
            }
        }
    }
    if( !bEndMonster )
        ...
        if( !bEndItem )
        {
            ...
            else if( strncasecmp( szLine, "color", 5 ) == 0 )
            {
                ...
                if( strchr( color, '<' ) != NULL )
                {
                    // multi-hued
                    // <<rgb1>,<rgb2>,...,<rgbn>>
                    JLog( LOG_LEVEL_INFO, "Found multi-hued item: %s\n", color );
