...
void CPlayer::PickUp( JVector &vPickupPos )
{
    ...
    if( pItem )
    {
        if( pItem->IsStackable() )
        {
            ...
            while( pExists != NULL )
            {
                ...
                if( Util::jstrcmp( pExists->m_lpData->GetName(), pItem->GetName() ) == 0 )
                {
                    ...
                    AILog_Event( "ITEM pickup name:%s count:%d pos:%d,%d",
        ...
        AILog_Event( "ITEM pickup name:%s count:1 pos:%d,%d",
...
int CPlayer::Move( JVector vDir )
{
    ...
    case DUNG_COLL_NO_COLLISION:
        m_vPos = vPos;
        AILog_Event( "MOVE player from:%d,%d to:%d,%d result:floor",
                     (int)vOldPos.x, (int)vOldPos.y, (int)vPos.x, (int)vPos.y );
        break;
    case DUNG_COLL_ITEM:
        m_vPos = vPos;
        AILog_Event( "MOVE player from:%d,%d to:%d,%d result:item",
                     (int)vOldPos.x, (int)vOldPos.y, (int)vPos.x, (int)vPos.y );
    ...
    if( dwCollideType == DUNG_COLL_MONSTER )
    {
        ...
        if( bHit )
        {
            ...
            if( pMon->TakeDamage( fDamage ) == STATUS_DEAD )
            {
                ...
                AILog_Event( "COMBAT attacker:Player defender:%s hit:true damage:%d hp:0 killed:true",
            ...
            {
                AILog_Event( "COMBAT attacker:Player defender:%s hit:true damage:%d hp:%d killed:false",
                             AILog_Name( szMonster ), (int)fDamage, (int)pMon->m_fCurHP );
            }
        }
        else
        {
            AILog_Event( "COMBAT attacker:Player defender:%s hit:false damage:0 hp:%d",
...
int CPlayer::TakeDamage( float fDamage, const char *szMon )
{
    ...
    Util::jstrcpy( m_szKilledBy, szMon );
        ...
        AILog_Event( "DEATH killed_by:%s level:%d pos:%d,%d",
