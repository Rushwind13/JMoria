...
{
    JLog( LOG_LEVEL_INFO, "Trying to spawn player..." );
    JVector vTryPos;
    while( !m_bHasSpawned )
    {
        ...
        JLog( LOG_LEVEL_DEBUG, "Trying to spawn player at <%.2f %.2f>...\n", vTryPos.x, vTryPos.y );
...
void CPlayer::DisplayInventory( uint8 dwPlacement )
{
    ...
    case PLACEMENT_USE:
        ...
        break;
    default:
        JLog( LOG_LEVEL_INFO, "DisplayInventory got bad placement: %d\n", dwPlacement );
        return;
        break;
    }
...
void CPlayer::DisplayEquipment( uint8 dwPlacement )
{
    ...
    case PLACEMENT_USE:
        ...
        break;
    default:
        JLog( LOG_LEVEL_INFO, "DisplayEquipment got bad placement: %d\n", dwPlacement );
        return;
        break;
    }
...
float CPlayer::Attack()
{
    ...
    JLog( LOG_LEVEL_INFO, "You rolled: %.2f, +to-hit Bonus: %.2f = Total: %.2f\n", fRoll, fHitMod,
          fRoll + fHitMod );
...
float CPlayer::Damage( float fDamageMult )
{
    ...
    JLog( LOG_LEVEL_INFO, "You did %.2f damage (damagemult: %.2f). ", fDamage, fDamageMult );
...
int CPlayer::TakeDamage( float fDamage, char *szMon )
{
    ...
    else
    {
        ...
        JLog( LOG_LEVEL_INFO, "%s died on dungeon level %d, while level %d, killed by a %s.\n",
